...
export async function GET() {
  try {
    ...
    const sql = db();
    const [documents, sessions, generations, reading] = await Promise.all([
      ...
      sql`select coalesce(sum(reading_seconds),0)::bigint as reading_seconds,
                 count(*) filter (where percent_complete >= 100)::int as completed_documents,
                 coalesce(avg(percent_complete),0)::numeric(5,2) as average_percent
          from public.document_reading_progress where user_id=${userId}`,
    ]);
    return NextResponse.json({ documents: documents[0], sessions: sessions[0], generations: generations[0], reading: reading[0] });
  } catch (error) {
