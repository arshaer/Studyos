...
const schema = {
  ...
  properties: {
    ...
    stages: {
      ...
      items: {
        ...
        required: ["title", "content", "check", "citations"],
  ...
  required: ["title", "objectives", "stages", "recap", "masteryQuestions"],
  additionalProperties: false,
};

async function ownedContext(userId: string, taskId: string) {
  const sql = db();
  const rows =
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const sql = db(),
      { item, chunks } = await ownedContext(userId, String(taskId));
    const existing =
    ...
    const profiles =
      await sql`select * from public.user_profiles where user_id=${userId}`;
    const course =
      await sql`select current_level,study_style,preferred_language from public.tutor_profiles where user_id=${userId} and document_id=${item.document_id}`;
    ...
    const labels = [
      ...new Set(
        chunks.map((c) => citationLabel(String(item.original_name), c)),
      ),
    ];
    const source = chunks
      .map(
        (c) =>
          `[SOURCE: ${citationLabel(String(item.original_name), c)}]\n${c.content}`,
      )
      .join("\n\n---\n\n");
    const prompt = `Teach the selected university section as an excellent private professor. Do not copy, dump, or merely summarize the source. Build progressively from prerequisites, use mechanisms, cause/effect, analogies, examples, comparisons of confusable concepts, exam-relevant points, and a short comprehension check in every stage. Language: ${language}; learner level: ${pref.academic_level || pref.current_level || "beginner"}; study style: ${pref.study_style || "mixed"}; depth: ${pref.explanation_depth || "adaptive"}. Preserve useful Italian technical terms. Section: ${item.section_title}, pages ${item.page_start}-${item.page_end}.`;
    ...
    const inserted =
      await sql`insert into public.professor_lessons(user_id,document_id,section_id,task_id,stages_json,mastery_questions_json,provider,model,input_tokens,output_tokens)values(${userId},${item.document_id},${item.section_id},${item.id},${JSON.stringify({ title: value.title, objectives: value.objectives, stages: value.stages, recap: value.recap })}::jsonb,${JSON.stringify(value.masteryQuestions)}::jsonb,${generated.provider},${generated.model},${generated.usage.input_tokens},${generated.usage.output_tokens}) on conflict(user_id,document_id,section_id) do nothing returning *`;
    const saved =
      inserted[0] ||
      (
        await sql`select * from public.professor_lessons where user_id=${userId} and document_id=${item.document_id} and section_id=${item.section_id}`
    ...
    console.error("Professor generation failed", e);
    const safe = publicAiError(e, language);
    return NextResponse.json(
      {
        error: safe.message,
        code: safe.code,
...
export async function PATCH(request: Request) {
  try {
    ...
    const body = await request.json(),
      sql = db();
    const owned =
      await sql`select * from public.professor_lessons where id=${String(body.lessonId)} and user_id=${userId}`;
    if (!owned[0])
      return NextResponse.json({ error: "Lesson not found" }, { status: 404 });
    const lesson = owned[0];
    if (body.action === "stage") {
      const stage = Math.max(0, Number(body.currentStage));
      const total = Array.isArray(lesson.stages_json?.stages)
        ? lesson.stages_json.stages.length
        : 0;
      const status = stage >= total ? "mastery_pending" : "learning";
      const rows =
        await sql`update public.professor_lessons set current_stage=${stage},status=${status},updated_at=now() where id=${lesson.id} and user_id=${userId} returning *`;
      await sql`update public.study_plan_tasks set learning_status=${status === "mastery_pending" ? "lesson_completed_mastery_pending" : "learning_in_progress"} where id=${lesson.task_id} and user_id=${userId}`;
    ...
    if (body.action === "mastery") {
      const answers = Array.isArray(body.answers) ? body.answers : [],
        questions = Array.isArray(lesson.mastery_questions_json)
          ...
          : [];
      const provider = configuredAiProvider("error_correction", {
        userId,
      ...
      const assessment = await provider.generate({
        mode: "tutor",
        prompt: `Grade these active-recall answers against the expected answers. Return JSON with score 0-100 and weakConcepts. Questions: ${JSON.stringify(questions)} Answers: ${JSON.stringify(answers)}`,
        schema: {
          type: "object",
          properties: {
            score: { type: "number" },
    ...
    console.error("Professor update failed", e);
    const safe = publicAiError(e);
    return NextResponse.json(
      {
        error: safe.message,
        code: safe.code,
