...
export async function GET(request: Request) {
  ...
  const [configRows, totals, periods, daily, byFeature, byProvider, byUser, sessions] = await Promise.all([
  ...
  return NextResponse.json({ from, to, config, totals: totals[0], periods: periods[0], daily, byFeature, byProvider, byUser, sessions: sessions[0], monthlyCostEur: Number(month[0]?.usd || 0) * usdToEur });
}
