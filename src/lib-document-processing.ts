...
type Heading={title:string;page:number;level:number;confidence:number;source:"outline"|"heuristic"};
...
export function buildSections(pages:Page[],outline:Heading[],documentTitle:string):Section[]{
  const candidates=outline.length?outline:pages.flatMap(page=>page.headings);const deduped=candidates.filter((heading,index)=>index===candidates.findIndex(other=>other.page===heading.page&&other.title.toLowerCase()===heading.title.toLowerCase()));
  const roots=deduped.length?deduped:[{title:documentTitle,page:1,level:1,confidence:.35,source:"heuristic" as const}];const ordered=roots.sort((a,b)=>a.page-b.page||a.level-b.level).map((heading,orderIndex)=>({...heading,id:randomUUID(),parentId:null as string|null,orderIndex,pageEnd:pages.at(-1)?.pageNumber||heading.page,charStart:0,charEnd:pages.at(-1)?.charEnd||0}));const stack:Section[]=[];
...
export async function processDocument(documentId:string,userId:string){
  ...
  for(const s of sections)await sql`insert into public.document_sections(id,document_id,user_id,parent_id,kind,level,title,order_index,page_start,page_end,char_start,char_end,confidence,source)values(${s.id},${documentId},${userId},${s.parentId},${s.level===1?"chapter":s.level===2?"section":"subsection"},${s.level},${s.title},${s.orderIndex},${s.page},${s.pageEnd},${s.charStart},${s.charEnd},${s.confidence},${s.source})`;
