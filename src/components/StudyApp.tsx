...
type Section = "dashboard" | "session" | "library" | "tutor" | "professor" | "doubt" | "summary" | "flashcards" | "questions" | "exams" | "progress" | "profile";
...
function inlineRichText(value:string){return value.split(/(\*\*[^*]+\*\*|__[^_]+__|`[^`]+`)/g).filter(Boolean).map((part,index)=>part.startsWith("**")&&part.endsWith("**")||part.startsWith("__")&&part.endsWith("__")?<strong key={index}>{part.slice(2,-2)}</strong>:part.startsWith("`")&&part.endsWith("`")?<code key={index}>{part.slice(1,-1)}</code>:<span key={index}>{part}</span>)}
function plainAiText(value:string){return value.replace(/^\s{0,3}#{1,6}\s*/g,"").replace(/\*\*([^*]+)\*\*/g,"$1").replace(/__([^_]+)__/g,"$1").replace(/^[-*]\s+/,"").trim()}
function RichAiText({content}:{content:string}){const lines=content.replace(/\r/g,"").split("\n"),nodes=[] as React.ReactNode[];let index=0;while(index<lines.length){const line=lines[index].trim();if(!line){index++;continue}const heading=line.match(/^(#{1,6})\s+(.+)$/);if(heading){const text=inlineRichText(heading[2]);nodes.push(heading[1].length<=2?<h3 key={index}>{text}</h3>:<h4 key={index}>{text}</h4>);index++;continue}if(/^[-*]\s+/.test(line)){const items=[] as string[];while(index<lines.length&&/^[-*]\s+/.test(lines[index].trim())){items.push(lines[index].trim().replace(/^[-*]\s+/,""));index++}nodes.push(<ul key={`ul-${index}`}>{items.map((item,itemIndex)=><li key={itemIndex}>{inlineRichText(item)}</li>)}</ul>);continue}if(/^\d+[.)]\s+/.test(line)){const items=[] as string[];while(index<lines.length&&/^\d+[.)]\s+/.test(lines[index].trim())){items.push(lines[index].trim().replace(/^\d+[.)]\s+/,""));index++}nodes.push(<ol key={`ol-${index}`}>{items.map((item,itemIndex)=><li key={itemIndex}>{inlineRichText(item)}</li>)}</ol>);continue}const paragraph=[line];index++;while(index<lines.length&&lines[index].trim()&&!/^(#{1,6})\s+|^[-*]\s+|^\d+[.)]\s+/.test(lines[index].trim())){paragraph.push(lines[index].trim());index++}nodes.push(<p key={`p-${index}`}>{inlineRichText(paragraph.join(" "))}</p>)}return <div className="rich-ai-text">{nodes}</div>}
function GeneratedHistory(){const[items,setItems]=useState<AiArtifact[]>([]),[selected,setSelected]=useState<AiArtifact|null>(null);useEffect(()=>{fetch("/api/ai").then(r=>r.json()).then(data=>setItems(data.artifacts||[])).catch(()=>{})},[]);return <div className="panel generated-history"><div className="panel-head"><div><b>Saved AI material</b><span>Reopen summaries, flashcards, questions, and exams</span></div></div>{!items.length&&<div className="empty-row">No saved generations yet.</div>}<div className="artifact-history">{items.map(item=><button key={item.id} onClick={()=>setSelected(item)}><b>{plainAiText(item.response_json?.title||item.mode)}</b><span>{item.document_title} · {item.provider}/{item.model}</span></button>)}</div>{selected&&<div className="artifact-reopen"><h3>{plainAiText(selected.response_json?.title||selected.mode)}</h3>{selected.response_json?.content&&<RichAiText content={selected.response_json.content}/>} {Array.isArray(selected.response_json?.items)&&<p>{selected.response_json.items.length} saved item{selected.response_json.items.length===1?"":"s"}. Open the matching study tool to practise this set.</p>}<small>{new Date(selected.created_at).toLocaleString()} · {selected.status}</small></div>}</div>}
...
type CoachTask={id:string;task_date:string;title:string;estimated_minutes:number;actual_minutes:number;status:string;learning_status?:string;page_start?:number;page_end?:number};
...
function Tutor({documents,startLesson}:{documents:StudyDocument[];startLesson:(task:CoachTask)=>void}){
...
function DoubtSolver({ documents, onStatusChange, openPdf }: { documents: StudyDocument[]; onStatusChange: () => void;openPdf:(document:StudyDocument)=>void }) {
  ...
  return <>
    ...
    <div className="tutor-grid"><div className="panel lesson-panel">
      ...
      <div className="lesson-body ai-output">{!result && !loading && <div className="ai-empty"><b>Ask your first question</b><span>StudyOS will answer using only the selected file.</span></div>}{loading && <div className="ai-loading">Reading your scoped material and preparing an answer…</div>}{result && <div className="tutor-answer"><h2>{plainAiText(result.title)}</h2><div className="lead"><RichAiText content={result.explanation}/></div>{result.definitions?.length>0&&<section><h3>Definitions</h3><dl>{result.definitions.map(item=><div key={item.term}><dt>{plainAiText(item.term)}</dt><dd>{plainAiText(item.definition)}</dd></div>)}</dl></section>}{result.steps?.length>0&&<section><h3>Steps</h3><ol>{result.steps.map(item=><li key={item}>{inlineRichText(item)}</li>)}</ol></section>}{result.keyPoints?.length>0&&<section><h3>Key points</h3><ul>{result.keyPoints.map(item=><li key={item}>{inlineRichText(item)}</li>)}</ul></section>}{result.examples?.length>0&&<section><h3>Examples</h3>{result.examples.map(item=><RichAiText key={item} content={item}/>)}</section>}{result.examCallouts?.length>0&&<aside className="exam-callout"><b>Exam focus</b>{result.examCallouts.map(item=><RichAiText key={item} content={item}/>)}</aside>}{result.tables?.map(table=><section key={table.title}><h3>{plainAiText(table.title)}</h3><div className="ai-table-wrap"><table><thead><tr>{table.headers.map(header=><th key={header}>{plainAiText(header)}</th>)}</tr></thead><tbody>{table.rows.map((row,index)=><tr key={index}>{row.map((cell,cellIndex)=><td key={cellIndex}>{inlineRichText(cell)}</td>)}</tr>)}</tbody></table></div></section>)}<div className="citation-list">{result.citations.map(source=><button className="source-chip" key={source} onClick={()=>openCitation(source)}>{source}</button>)}</div></div>}</div>
...
function Professor({task}:{task:CoachTask|null}){const[lesson,setLesson]=useState<any>(null),[section,setSection]=useState<any>(null),[busy,setBusy]=useState(false),[error,setError]=useState<{message:string;retryAfterSeconds?:number}|null>(null),[answers,setAnswers]=useState<string[]>([]),[result,setResult]=useState<any>(null);async function load(){if(!task)return;setBusy(true);setError(null);try{const r=await fetch('/api/professor',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({taskId:task.id})}),j=await r.json();if(!r.ok)throw j;setLesson(j.lesson);setSection(j.section)}catch(e:any){setError({message:String(e?.error||"The lesson could not be created. Your progress is safe; please retry."),retryAfterSeconds:Number(e?.retryAfterSeconds)||undefined})}finally{setBusy(false)}}useEffect(()=>{void load()},[task?.id]);if(!task)return <><PageTitle kicker="TEACH ME" title="Professor" text="Open an indexed item from your Tutor plan to begin a guided lesson."/><div className="panel empty-row">Choose Start lesson from Tutor.</div></>;const data=lesson?.stages_json,stages=data?.stages||[],index=Number(lesson?.current_stage||0),stage=stages[index],questions=lesson?.mastery_questions_json||[];async function advance(){const r=await fetch('/api/professor',{method:'PATCH',headers:{'content-type':'application/json'},body:JSON.stringify({lessonId:lesson.id,action:'stage',currentStage:index+1})}),j=await r.json();if(r.ok)setLesson(j.lesson)}async function mastery(){setBusy(true);const r=await fetch('/api/professor',{method:'PATCH',headers:{'content-type':'application/json'},body:JSON.stringify({lessonId:lesson.id,action:'mastery',answers})}),j=await r.json();if(r.ok)setResult(j);else setError({message:j.error});setBusy(false)}return <><PageTitle kicker="PROFESSOR · GUIDED LESSON" title={section?.section_title||task.title} text={`${section?.original_name||"Course"} › pages ${section?.page_start||task.page_start}–${section?.page_end||task.page_end}`}/>{busy&&!lesson&&<div className="panel ai-loading">Professor is designing your lesson. If the AI is busy, StudyOS will retry safely and use the configured fallback.</div>}{error&&<div className="panel ai-recovery"><b>{error.message}</b>{error.retryAfterSeconds&&<span>You can retry in about {error.retryAfterSeconds}s.</span>}<button className="primary" disabled={busy} onClick={()=>void load()}>{busy?"Retrying…":"Retry lesson"}</button></div>}{lesson&&<div className="tutor-grid"><div className="panel lesson-panel"><div className="lesson-head"><div><span>CURRENT CONCEPT</span><h3>{stage?.title||"Recap & mastery"}</h3></div><div className="mini-progress"><span>LESSON PROGRESS</span><b>{Math.round(Math.min(100,index/Math.max(1,stages.length)*100))}%</b></div></div><div className="lesson-body ai-output">{index===0&&<section><h3>Learning objectives</h3><ul>{(data.objectives||[]).map((x:string)=><li key={x}>{x}</li>)}</ul></section>}{stage?<><h2>{stage.title}</h2><RichAiText content={stage.content}/><div className="concept-box"><b>Comprehension check</b><p>{stage.check}</p></div><div className="citation-list">{stage.citations?.map((x:string)=><span className="source-chip" key={x}>{x}</span>)}</div><div className="quick-actions"><button>Explain simpler</button><button>Go deeper</button><button>Give example</button><button>Show comparison</button><button>Why?</button></div><button className="primary" onClick={()=>void advance()}>I understand · continue</button></>:<><h2>Recap, doubts & mastery</h2><ul>{(data.recap||[]).map((x:string)=><li key={x}>{x}</li>)}</ul><p>Anything unclear? Review the lesson or answer these active-recall prompts. Mastery is based on your answers.</p>{questions.map((q:any,i:number)=><label className="mastery-question" key={i}><b>{i+1}. {q.question}</b><textarea value={answers[i]||""} onChange={e=>setAnswers(a=>{const n=[...a];n[i]=e.target.value;return n})}/></label>)}<button className="primary" disabled={busy||answers.filter(Boolean).length<questions.length} onClick={()=>void mastery()}>{busy?"Assessing…":"Complete mastery check"}</button>{result&&<div className={`result ${result.status==="mastered"?"correct":"wrong"}`}><b>{result.score}% · {result.status.replaceAll('_',' ')}</b><span>{result.status==="mastered"?"Section mastered and Tutor progress updated.":"Targeted review recommended before completion."}</span></div>}</>}</div></div><div className="panel tutor-side"><b>Lesson stages</b><div className="topic-list">{stages.map((s:any,i:number)=><div className={i===index?"active":""} key={s.title}><span>{s.title}</span><b>{i<index?"✓":i+1}</b></div>)}</div></div></div>}</>}

function Profile({email}:{email:string}){const[p,setP]=useState<any>({displayName:"",appLanguage:"en",teachingLanguage:"it",studyStyle:"mixed",defaultSessionMinutes:25,breakMinutes:5,dailyStudyHours:2,explanationDepth:"adaptive",academicLevel:"beginner",examStyle:"mixed",theme:"system"}),[notice,setNotice]=useState(""),[confirm,setConfirm]=useState("");useEffect(()=>{fetch('/api/profile').then(r=>r.json()).then(j=>j.profile&&setP({displayName:j.profile.display_name||"",appLanguage:j.profile.app_language,teachingLanguage:j.profile.teaching_language,studyStyle:j.profile.study_style,defaultSessionMinutes:j.profile.default_session_minutes,breakMinutes:j.profile.break_minutes,dailyStudyHours:j.profile.daily_study_hours||"",explanationDepth:j.profile.explanation_depth,academicLevel:j.profile.academic_level,examStyle:j.profile.exam_style,theme:j.profile.theme}))},[]);async function save(){const r=await fetch('/api/profile',{method:'PUT',headers:{'content-type':'application/json'},body:JSON.stringify(p)});setNotice(r.ok?'Profile saved. Tutor and Professor will use these preferences.':'Could not save profile.')}async function remove(scope:string){const r=await fetch('/api/profile',{method:'DELETE',headers:{'content-type':'application/json'},body:JSON.stringify({scope,confirm})}),j=await r.json();if(!r.ok){setNotice(j.error);return}if(scope==='account'){const authResult=await authClient.deleteUser();if(authResult.error){setNotice(`StudyOS data was removed, but auth deletion failed: ${authResult.error.message}`);return}await authClient.signOut()}else{setNotice(scope==='learning'?'Learning data reset. Uploaded documents were kept.':'All StudyOS data and uploaded files were deleted.')}}return <><PageTitle kicker="ACCOUNT & PREFERENCES" title="Profile" text="Preferences here directly shape your study plan and Professor lessons."/><div className="profile-grid"><div className="panel profile-form"><h3>Account</h3><label>Display name<input value={p.displayName} onChange={e=>setP({...p,displayName:e.target.value})}/></label><label>Email<input value={email} readOnly/></label><h3>Study preferences</h3><label>Teaching language<select value={p.teachingLanguage} onChange={e=>setP({...p,teachingLanguage:e.target.value})}><option value="it">Italiano</option><option value="en">English</option><option value="fa">فارسی</option></select></label><label>Study style<select value={p.studyStyle} onChange={e=>setP({...p,studyStyle:e.target.value})}><option>mixed</option><option>active recall</option><option>explanation</option><option>flashcards</option><option>oral simulation</option></select></label><label>Explanation depth<select value={p.explanationDepth} onChange={e=>setP({...p,explanationDepth:e.target.value})}><option>adaptive</option><option>simple</option><option>deep</option></select></label><div className="session-fields"><label>Session<input type="number" value={p.defaultSessionMinutes} onChange={e=>setP({...p,defaultSessionMinutes:Number(e.target.value)})}/></label><label>Break<input type="number" value={p.breakMinutes} onChange={e=>setP({...p,breakMinutes:Number(e.target.value)})}/></label><label>Daily hours<input type="number" step=".5" value={p.dailyStudyHours} onChange={e=>setP({...p,dailyStudyHours:Number(e.target.value)})}/></label></div><h3>Academic profile</h3><label>Current level<select value={p.academicLevel} onChange={e=>setP({...p,academicLevel:e.target.value})}><option>beginner</option><option>intermediate</option><option>strong</option></select></label><label>Default exam style<select value={p.examStyle} onChange={e=>setP({...p,examStyle:e.target.value})}><option>mixed</option><option>oral</option><option>written</option></select></label><button className="primary" onClick={()=>void save()}>Save profile</button></div><div className="panel danger-zone"><h3>Data & Privacy</h3><p>Your uploaded material and study history belong to this account. Learning reset never removes source documents.</p><label>Type DELETE to enable destructive actions<input value={confirm} onChange={e=>setConfirm(e.target.value)}/></label><button disabled={confirm!=="DELETE"} onClick={()=>void remove('learning')}>Reset learning data · keep documents</button><button disabled={confirm!=="DELETE"} onClick={()=>void remove('all')}>Delete all StudyOS data · including files</button><button className="danger" disabled={confirm!=="DELETE"} onClick={()=>void remove('account')}>Delete account permanently</button>{notice&&<div className="upload-error">{notice}</div>}</div></div></>}
